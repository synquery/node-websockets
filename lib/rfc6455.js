...
function write(message, maskbit) {
  ...
  var i = 0, size = parent._options['bufferSize'];
  var fin = 0;
  var ext = [0, 0, 0];
  ...
  while(fin == 0) {
    var tail = Math.min(buf_len, i + size);
    fin = buf_len > tail ? 0: 1;
    process.nextTick(function(fin, f_data) {
      return function() {
        socket.write(frame(fin, ext, opcode, mask, f_data));
      };
    }(fin, buf.slice(i, (i = tail))));
  }
...
Client.prototype.handShake = function(uri) {
  ...
  if(secure) {
    ...
    opts['rejectUnauthorized'] = typeof rejectUnauthorized === 'boolean' ? rejectUnauthorized: false;
  }
