/*
 * [node-websockets](lib/rfc6455.js)
 * rfc6455 に基づいたプロトコル実装および、拡張
 * 2021/08/13 remove Buffer depercation 
 */
...
Rfc6455.prototype.accumulate = function(data, fin, opcode) {
  ...
  if(0 !== opcode)
    rfc6455.opcode = opcode, rfc6455.buffer = Buffer.alloc(0); // new Buffer(0);
  else if(!rfc6455.buffer)
    return; // overflowing frames

  var bufd = Buffer.isBuffer(data) ? data: Buffer.from(data); // new Buffer(data);
...
function _ctrl(opcode, maskbit, data) {
  ...
  var parent = this.parent, buf = Buffer.isBuffer(data) ? data: Buffer.from(data); // new Buffer(data);
  if(125 < buf.length)
    buf = buf.slice(0, 124);
...
function write(message, maskbit) {
  ...
  var buf = Buffer.isBuffer(message) ? message: Buffer.from(message); // new Buffer(message);
...
function parse(data, maskbit) {
  ...
  var OP_FUNC = {
    ...
    0x0A: function pong(payload) {
      ...
      parent.emit('pong', payload.toString());
    }
  ...
  if('undefined' === typeof self.buffer) { // condition to be parse
    self.buffer = _parseHead();
  }

  if(!Buffer.isBuffer(self.buffer)) { // condition after _parseHead
    _parsePayload();
  }
  ...
  function _parseHead() {
    ...
    var mask = _byte >>> 7;
    if(maskbit !== mask) {
      parent.emit('error', 1002);
      console.log('Illegal maskbit:' + mask, ', expected:' + maskbit);
      return; // Should return undefined for next buffer
    }
    
    var len = _byte & 0x7F;
    if(length < 2 + (({ "126": 2, "127": 8 })[len] || 0) + (maskbit ? 4: 0)) {
      return data;
    }
  ...
  function _parsePayload() {
    ...
    // Illegal payload ( may self.buffer = data or Buffer(0) )
    if(payload == NULL) {
      console.log('Illegal payload:', buf, data);
      return;
    }
...
Server.prototype.handShake = function(req, socket, upgradeHead) {
  ...
  socket.write(Buffer.from(HANDSHAKE_RESPONSE + newkey + '\r\n\r\n'));
