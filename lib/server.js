...
function WebSocketServer(options) {
  ...
  self.on('upgrade', function(req, socket, upgradeHead) {
    ...
    var _socket = conn._socket = socket.socket || socket.soket || socket;
    ...
    conn.on('configure', function(type, value) {
      switch(type) {

      case 'site':
        // register my group for mutual broadcast
        // remove from the position now.
        var internal = connections[site] || [];
        var index = internal.indexOf(conn);
        0 <= index && internal.splice(index, 1);
        // push to the new position
        site = value;
        internal = connections[site] || (connections[site] = []);
        internal.push(conn);
        return;
  ...
  self.on('broadcast', function(message, options) {
    ...
    var exec = typeof options.exec == 'function' ? options.exec
      : function(conn, message, type) {
